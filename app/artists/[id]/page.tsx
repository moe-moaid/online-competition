...
export default async function Page({
  ...
  const user = await getSingleArtist(userId);

  if (!user.artist) {
    return (
      <p className="mx-auto py-10 text-center text-2xl font-semibold text-white">
        No <span className="text-legendary">Artist</span> Found!
      </p>
    );
  }

  return (
    <Suspense fallback={<Loading resource="user" />}>
